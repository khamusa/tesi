...
\textit{layers}: c'era un \textit{layer} ``PORTE'', un \textit{layer} ``RM\$'' che conteneva i poligoni delle stanze, un \textit{layer} ``FINESTRE'', e così via. Ma in qualche file, Il layer ``PORTE'' potrebbe chiamarsi ``PORTA'', le finestre potrebbero essere state inserite insieme alle murature nel \textit{layer} ``MURI'', mentre il \textit{layer} ``SCALE'' era vuoto con le scale disegnate insieme alle stanze su ``RM\$''. Sul \textit{layer} delle porte, a volte trovavamo le porte disegnate come linee e archi sparsi, a volte raggruppate all'interno di oggetti composti (chiamati \textit{Insert}, una specie di gruppo di oggetti), e lo stesso valeva per le finestre e le scale. 
...
REPL, con l'interprete python bpython\footnote{\url{http://www.bpython-interpreter.org/}}, che fornisce diverse funzionalità utili per un approccio del genere:
...
e \textit{testing} non erano la priorità. In questa fase di \textit{refactoring} dovevamo sanare queste eventuali deficienze.
...
\mbox{
\pythonexternal[caption=Versione
iniziale del metodo update\_buildings,frame=tlrb,numbers=left]{code_samples/building_data_updater_before.py}
}
...
è una forte componente dell'esperienza di uno sviluppatore. Inoltre,ç partendo
...
Essendomi particolarmente interessato per l'argomento, ho trovato un libro 
...
Riflettendo su quei nuovi contenuti, mi sono accorto del
banale errore che avevamo commesso, e mi 
...
Si noti inoltre il rilievo dato da Fowler nell'ultima frase citata
all'idea di \textit{Refactoring Regolare}. Su questo aspetto ne discute
ancora:
...
Assumendo allora che questo approccio di \textbf{continuous refactoring}
portato in avanti fin dall'inizio di un progetto, sono 
...
Vorrei sottolineare che esiste una netta differenza tra i due contesti
...
a ugualmente o più elegante? Su questa tipologia di refactoring,
...
le dipendenze'', ``disaccoppiare moduli'', eccettera.
...
falliti o con successo soltanto parziale, e allora tanta riflessione
pratica, la letteratura e la teoria sull'argomento mi hanno aiutato a 
portare il ragionamento in avanti.

Una volta sono riuscito a compiere questi passi, ho cercato di riapplicare
...
priori, perché questo viene fatalmente eroso man mano riceve modifiche
...
raccontato concludo inoltre che il procedere a piccoli passi ci conferisce
anche sicurezza sulle modifiche nel design. A piccoli passi esiste una
maggior probabilità che ce ne accorgiamo quando il refactoring è già
sufficiente. Io invece avevo fatto quelle modifiche giganti, blah blha blah

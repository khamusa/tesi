...
fanno si che molto spesso diventi impossibile la manutenzione continuata
della qualità del software, a meno ovviamente di una forte resistenza 
da parte degli sviluppatori coinvolti, il che rischia addirittura di danneggiare
i rapporti professionali. È allora più naturale che in questo contesto la
preoccupazione con il design e la qualità del codice decada a misura che ci
avviciniamo alle scadenze, esperienza che ho vissuto più volte.

Su questo progetto sono partito 
con il chiaro scopo di curare bene il \textit{design}, ero
...
Nel corso di questo progetto gli effetti della sistematica ma speculativa 
rincorsa del design sono stati positivi in determinati ambiti, 
mentre su altri ci hanno generato moduli complessi e con eccessive indirezioni. 
Cercando di anticipare le richieste future, abbiamo predisposto una serie di
fondazioni che poi non abbiamo effettivamente utilizzato. Abbiamo allora
ottenuto più codice da mantenere, e una maggior difficoltà della 
comprensione dei flussi di esecuzione.

Su questa problematicità vorrei citare le riflessioni di Kent Beck
...
\begin{quote}
	...
	Se si conclude il lavoro del giorno, ma se lo fa in modo che diventi 
	...
	sicuri sul domani. Forze faremo questo, forze quell'altro, o forze qualcosa
...
Se da un lato Kent Beck avverte contro il rischio di implementare le 
...
grande probabilità, non si pagherà: infatti, le anticipazioni a che
possiamo provvedere sono trascurabili rispetto a tutti i possibili
cammini di sviluppo che al sistema potrebbero capitare.
...
\begin{quote}
	...
	si cambia quella decisione. Allora si può fare il lavoro di oggi. Domani,
	qualche delle comprensioni di oggi sembreranno ingenue, e allora
...
\begin{quote}
	Si deve comparare questo (il refactoring) con il curato design
	a priori. Il design speculativo è un tentativo di includere
	tutte le qualità positive nel sistema prima ancora che qualsiasi
	codice sia stato scritto. Il problema di questo procedimento è
	che è molto facile indovinare in modo sbagliato. Con il refactoring,
	non si ha il rischio di essere completamente sbagliati. Alla fine
	il programma si comporta sempre come all'inizio.
...
accogliere l'effimerità delle scelte attuali, di sapere che, con grande
...
più semplice possibile, limitando gli scarti futuri. L'aggiunta di 
indirezione o altri livelli di astrazione va fatta dopo, quando 
effettivamente viene verificata la necessità. L'aggiunta di
astrazione va fatta a partire dal refactoring, 
senza che porti dei cambiamenti sul comportamento del
sistema. Si basa allora su un codice già funzionante e su tutta la rete
di protezione fornita dalla suite di test previamente definita. 
...
ci permette allora di circoscrivere la complessità del sistema alle sole

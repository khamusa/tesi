...
che ho chiamato \textit{Experimentation Driven Development}. %PROF: strettamente imparentato con prototyping direi...
...
Il nostro \textit{workflow} di base era composto dai seguenti passi,
...
dove si trovasse la specifica Aula/stanza di suo interesse all'interno
di un palazzo, oppure indicare la localizzazione di stanze a seconda
...
L'ottenimento del formato (grafico) delle stanze non è stato  
...
Abbiamo elaborato i test a posteriori, per aumentare la confidenza 
...
suo disegno era la loro  posizione relativa (cioè la posizione dell'etichetta all'interno del poligono della stanza), senza altre indicazioni a
...
%PROF: argomento di discussione: 
%Perché ad esempio in questo caso non avete fatto TDD sulla ipotesi?
%Mi spiego... la domanda hai deto che era chiara. Il punto è dentro al poligono.
%la soglia di precisione voluta è fissata.
%Era ipotizzabile scrivere un test in cui dicevo che chiamando l'algoritmo su 
%tutte (o un campione significativo delle) le
%stanze, al 90% di esse avrei assegnato una etichetta... 
%non avete mai implementato neanche a posteriori il soddisfacimento delle 
%soglie come test vero?
...
la nostra ipotesi poteva effettivamente essere considerata verificata.
...
Questo è un esempio di una metodologia che abbiamo utilizzato più volte, 
specialmente nell'affrontare l'estrazione e comprensione dei dati originati
...
troppo alto se mai fosse stato necessario tornare in dietro.
...
Avere un insieme di test di cui ci fidavamo è stato essenziale per
garantire l'avanzamento dello sviluppo, ma
non era sufficiente per metterci al riparo dal deterioramento del
...
Gli esempi forniti fino a questo momento e la parte del processo descritta
...
Non bastava esserci posti l'obbiettivo della ricerca costante della qualità e 
...
grossa differenza, e l'esperienza e la pratica sono ciò che molto spesso ci 
rende in grado di stabilire una diagnosi completa.
...
ad individuare i problemi.  Mi mancava l'esperienza, 
...
design in Ruby: an agile primer'': %PROF: aggiungere in bibliografia e citare.
...
Un esempio pratico in cui l'SRP ha contribuito a due livelli di granularità diversi è la
...
(file CSV). La sua interfaccia era costituita da due metodi principali, 
update\_buildings e update\_rooms (aggiorna palazzi e stanze 
rispettivamente).
...
Di conseguenza, DataUpdater era superclasse di due classi più specializzate: 
...
``compiuta'' quell'impresa. %PROF: non ho capito... sei arrivato alla fine della giornata SENZA avere finito?
Ero stanco e dovevo ancora risistemare un paio
...
in modalità \textit{ad hoc}, cercando di ottenere un equilibrio instabile tra concetti
...
un po' estremo, e che più spesso conviene analizzare caso per caso. La
...
anche per il fatto che si trattava di implementazioni molto fragili,
...
\begin{itemize}
  ...
  pagina di codice e permette con un unico sguardo di coglierne il flusso.
  ...
  }, %PROF: non vedo il collegamento con le punchcards... più che altro era la dimensione dei monitor alfanumerici classici e delle stampanti
  per aumentare la leggibilità e far sì che sia possibile visualizzare
...
\begin{itemize}
  ...
  testando i singoli \textit{metodi} 
  che lo compongono, e poi verificando che le
  chiamate interagiscano nel modo corretto. %PROF: tolte perché il mapping con "conquer" era molto forzato volendolo esplicitare. Allora sarebbe stato meglio Cartesio.
...
il suo scopo, senza necessità di ausili esterni, nè di commenti, nè di documenti
...
veramente poco. La ricerca della perfezione va contestualizzata, altrimenti diventa
dannosa. %PROF: in italiano spesso si usa dire "il meglio è nemico del bene"
...
succede, come ad esempio vendte di palazzi, palazzi che non sono di proprietà 
dell'università ma che le vengono temporaneamente dati in uso (e quindi devono essere tracciati). 
...
a piccoli passi, ma di sicuro ci sarebbe stata una maggiore possibilità 
di non dover arrivare alla mattina per accorgermene.

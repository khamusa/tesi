...
%%PROF: ABBIAMO SCRITTO "CONTINUOS"???  PERCHE' DOVEVA ESSERE "CONTINUOUS" !!!
...
essere facilmente trovate in rete. %% PROF: se lo hai fatto, aggiungi "nelle risorse che di volta in volta ho citato in bibliografia. "
...
o disambiguare la traduzione.  %%PROF: probabilmente toglierei la traduzione e lascerei versione inglese (ma confermerò quando le leggo)
...
A questo concetto do il nome di \textit{“continuous refactoring”}\footnote{Si veda il capitolo \ref{cap4} e, ancora di più, il capitolo \ref{cap5}}.
...
\label{cap2} %%PROF: per il futuro ricordati che in latex non ha senso mettere una label con il numero del capitolo... ad esempio se decidessi di spostarlo, tutte le citazioni risulterebbero fuorvianti ... e' meglio usare label sul contenuto, tipo: \label{att_preliminari}
...
elaborazione. Le sorgenti dati che dovevamo integrare erano principalmente due: la divisione Manutenzione edilizia e impiantistica, e la divisione Sistemi informativi. I documenti forniti a loro volta possono essere suddivisi in più tipologie di informazione e
...
\textit{``CAD-compatible''} definito proprio per le operazioni di interscambio) utilizzando uno strumento fornito gratuitamente da Autodesk. I file DXF sono stati quindi processatitramite la libreria dxfgrabber \footnote{Libreria \textit{opensource},
sorgente disponibile in https://bitbucket.org/mozman/dxfgrabber}. Più avanti parlerò anche delle motivazioni per le scelte
...
costruiti di quei dati incominciò a rivelare i suoi difetti: errori di battitura, utilizzo di diversi
...
semantico. %%secondo me il seignificato semantico c'era anche prima, ora si aggiunge quello di coerenza/consistenza
...
generazione di immagini, ecc);
  \item La positiva sfida rappresentata dal fatto che Python era un linguaggioquasi nuovo per tutti i partecipanti al progetto;
  \item La convinzione che la scelta di Python non sarebbe  stata limitante per la continuazione del progetto da parte di altri studenti/tesisti, in quanto è
...
il processo di sviluppo. %%soprattutto se gli sviluppatori non ne sonon esperti
...
%%PROF: La bibliografia non va fatta così... va usato bibtex... ci metti pochi minuti a capirlo, evita errori stupidi e fattorizza il lavoro.

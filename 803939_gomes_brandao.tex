...
Dati i tempi stretti, nella maggior parte dei casi abbiamo optato per la rielaborazione del prototipo esistente e non per il suo scarto, poiché era evidente che una nuova versione fatta da zero avrebbe avuto molto in comune con il prototipo in mano. Infatti questa è la maggior differenza del nostro approccio a quelli prototipali più comuni, in cui si cerca di trattare i prototipi in modo usa-e-getta. Nella rielaborazione, ci siamo assicurati di:
...
diventava vittima del fenomeno noto come software decay. La nostra intesa ``
buona progettazione'' fatta dall'inizio veniva poco a poco storpia.
...
Anche se spesso i singoli pezzi/moduli erano fatti bene, con un opportuno 
isolamento, il codice sembrava quello che viene chiamato \textit{ravioli code}
\footnote{TODO: comparare ravioli e spaghetti}, 
cioè fatto da piccoli pezzettini bene isolati e modularizzati ma la cui 
interrelazione componeva un sistema ancora caotico e senza ordine.

Sentivo ovunque i \textit{code smells}
\footnote{letteralmente ``puzza del codice''}, cioè sintomi sul programma che indicavano 
...
\section{100 Programmatori in Cerca di Esperienza}

In mancanza di questa figura all'interno del team, mi chiedevo in che altro modo 
avrei potuto acquisire quell'esperienza. Il primo tentativo che ho considerato un
successo parziale è stato quello di cercare i concetti teorici della 
``buona progettazione'' e in particolare sono finito a rileggermi ancora una 
volta i principali materiali disponibili sui principi SOLID\footnote{TODO}. 

Dei principi SOLID, il \textit{Single Responsibility Principle}
...
Per applicarlo, ho seguito un suggerimento di ... L'idea è quella di guardare
le classi o metodi esistenti e cercare di sintetizzare in una frase quale sia
la sua responsabilità. In seguito, si deve cercare di contare quante volte la
congiunzione ``e'' e ``oppure'' compaiono. Un numero elevato di queste 
congiunzioni va considerato come un forte indizio che quella classe o metodo
gestisce più di quanto ne deve.

Anche se inizialmente in concetto è stato concepito con granularità di classe,
ho cercato di applicarlo a diversi livelli. 
...
questioni di compatibilitàà, mentre quelli usati da EasyRoom avevano un solo formato.

In sostanza, il \textit{class diagram} era come segue\footnote{Per semplicità ho ommesso
metodi e attributi privati}:

[TODO \textit{Class Diagram} iniziale]

Alla stesura della sua ``frase di responsabilità'', ho ottenuto il seguente: 
``Gestire la validazione, aggiornamenti, integrazione dei dati sulle stanze 
e/o palazzi e conseguente inserimento in database''. 
Dalla lunghezza della frase e dalla quantità di 
congiunzioni, ho concluso che si trattava di una classe da venir spezzata.
Si trattava di una classe considerevolmente lunga, e sicuramente ne gestiva almeno
...
sarebbe quella di favorire la composizione invece di usare l'ereditarietà. 
Ho scelto per l'ereditarietà per due motivi: non avevamo richieste di 
...
\begin{itemize}
  \item RoomDataUpdater: ``Gestire la validazione, aggiornamenti, 
  integrazione dei dati delle stanze e inserimento in database''
  \item BuildingDataUpdater: ``Gestire la validazione, aggiornamenti, 
  integrazione dei dati dei palazzi e inserimento in database''
\end{itemize}

Non ero soddisfatto. Avevo messo su il mio ``cappello del refactoring'', e non
lo volevo togliere (mi stava bene :D). Ho incominciato con RoomDataUpdater,
e ho provato ulteriori suddivisioni, cercando dei pattern 
da applicare e sono riuscito a fare un paio di prove. Alla fine di questa
...
La mia fortuna è che si trattava di fine giornata quando ho dichiarato 
...
testavano l'esecuzione precedente in modo vincolato all'implementazione.
Il giorno dopo, ansioso per il primo \textit{commit} mattinale,
al guardare ciò che avevo fatto ho sentito: puzzava. Mi sono messo a leggere
ancora altri concetti di Progettazione Software, e un po' per caso ho riletto
sul concetto di \textit{Cohesion}\footnote{La coesione è una misura di 
...
con il termine \textit{coupling}, che sta ad indicare un accoppiamento
fra moduli/classi non necessario eche genera più dipendenze e difficoltà
...
temporale di quell'algoritmo. Il motivo per cui il metodo update\_rooms
...
identificativo del palazzo a cui appartiene). In seguito avrei preparato il
...
procedere con la parte di integrazione con altre sorgenti già presenti. 

Era un modulo in cui gli elementi godevano di coesione di tipo:
\begin{itemize}
...
all'interno della stessa classe. Ancora una volta l'argomento ``Progettazione
Software'' mi ha fatto capire quanto sia importante analizzare ogni caso
in modalità ad hoc, cercando di ottenere un equilibrio instabile tra concetti
diversi e come una loro applicazione ingenua può portarli pure a contraddirsi.

Ma non ero ancora soddisfatto. C'era qualcosa che, ancora, non andava bene!

\section*{Granularità diversa}

[ Adesso: come ho applicato SRP con granularità di singolo algoritmo e le
conseguenze che ha avuto su RoomDataUpdater ]

Una volta che il file csv di stanze veniva letto, alla classe DataUpdater 
veniva delegato il lavoro di eseguire veramente l'aggiornamento, con 
la chiamata del metodo statico update\_rooms, che riceveva appunto la 
lista di informazioni da processare e finalmente inserire nel database. 
Abbiamo sintetizzato un algoritmo che prevvedeva alla validazione, 
pulizia e integrazione dei dati con le altre sorgenti già presenti in database (ad esempio le piante 
architettoniche). Tale algoritmo funzionava bene e godeva di una minima quantità
di test di unità e di accettazione. 

Il codice però
L'algoritmo risultante funzionava bene ma era lungo e difficile da capire.
...
Alla \textit{Foreword} scritta da Erich Brooch
\footnote{TODO: dire chi è erich brooch} sentivo di aver trovato un punto di 
...
buona progettazione não é só a priori, mas também com refactoring, continuos refactoring porca madonna!
(TODO: Dire che in letteratura / in quel libro ho trovato la figura che prima volevo: la voce dell'esperienza hahahah )
(TODO: prima pensavo a grandi cambiamenti strutturali, cose che avrebbero ``risolto'' il problema del design. Loro invece parlano di piccoli step.)
(TODO: dopo aver applicato ho applicato anche ai test, e alla fine corretto la quantità di test tautologici aggiunti in precedenza)
(TODO: usare la repl non è sabagliato, ma alla fine ho concluso che per ogni fase / momento ci vuole anche un workflow diverso!)
